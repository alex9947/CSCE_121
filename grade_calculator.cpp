...
int main() {
    ...
    double exam_average = exam/ex_num;
    if (exam_average < 60) {
      ex_num += 1;
    }
    double final_average = (final + exam)/(ex_num + fin_num);
    if (final >= exam_average) {
      exam_average = final;
    } else {
      exam_average = final_average;
    }
    double hw_average = hw/hw_num;
    double lw_average = lw/lw_num;
    double engagement = 0;
